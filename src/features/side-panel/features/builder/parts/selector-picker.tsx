 ...
 * via PICK_RESULT and fills this input. The SelectorInspector panel shows
 * stability score and fallbacks after a successful pick.
 */

import { useEffect, useState } from 'react'
import { MSG } from '@shared/constants/messages'
import { SelectorInspector } from './selector-inspector'
...
export function SelectorPicker({ value, onChange, placeholder = 'CSS or XPath selector', className = '' }: Props) {
  ...
  const [inspected, setInspected] = useState<SelectorObj | null>(null)
  ...
  useEffect(() => {
    ...
    const handler = (message: Record<string, unknown>) => {
      ...
      if (selector) {
        onChange(selector)
        const obj = message['selectorObj'] as SelectorObj | undefined
        if (obj) setInspected(obj)
      }
      setPicking(false)
    }
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-stretch">
        <input
          className={inputCls}
          value={value}
          onChange={e => { onChange(e.target.value); setInspected(null) }}
      ...
      {inspected && (
        <SelectorInspector
          data={{
            ...
            fallbacks: inspected.fallbacks,
            stabilityScore: inspected.stabilityScore,
            tagName: inspected.tagName ?? 'element',
            text: inspected.text ?? '',
          }}
          onClose={() => setInspected(null)}
          onUseSelector={sel => { onChange(sel); setInspected(null) }}
        />
      )}
    </div>
  )
